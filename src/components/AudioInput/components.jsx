...
import { FolderIcon, LinkIcon, MicrophoneIcon, XIcon } from "./icons";
...
export const AudioSourceSelector = ({
  ...
  <div className="bg-slate-800/30 rounded-xl border border-slate-700/50 p-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      {allowedModes.includes("record") && navigator.mediaDevices && (
        <AudioTile
          icon={<MicrophoneIcon />}
          title="Record"
    ...
    {currentAudio && <AudioIndicator audio={currentAudio} onClear={onClearAudio} />}
  </div>
);
...
export const FileInputMode = ({ onClose, onChange, error }) => (
...
export const UrlInputMode = ({ url, setUrl, onSubmit, onClose, loading, error }) => (
  <div className="space-y-4">
...
export const RecordingMode = ({
  ...
  <div className="space-y-4">
    <ModeHeader title="Record Audio" onClose={onClose} />
    <div className="text-center space-y-4">
      {!recording && !recordedBlob && <RecordButton onClick={onStart} />}

      {recording && <RecordingIndicator duration={formatDuration(duration)} onStop={onStop} />}
...
export const RecordingPreview = ({ blob, onRerecord, onAccept }) => (
  <>
    <AudioPlayer audioUrl={URL.createObjectURL(blob)} title="Recording Preview" />
